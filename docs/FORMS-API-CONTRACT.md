...
There are **5 forms** on the site. Each form is currently **static** (frontend only) — no form currently sends data to a backend. The submit buttons simply `preventDefault()` and toggle a "submitted" state. Below is the exact field inventory and the payload contract the backend should be built against.
...
| Field (UI Label) | HTML type | `name` attribute (to add) | Required | Format / Notes |
|------------------|-----------|---------------------------|----------|----------------|
| Your Name | `text` | `full_name` | ✅ Yes | string, max 100 chars |
| Mobile Number | (text) | `mobile_number` | ✅ Yes | string, 10–15 digits, allow `+` prefix |
...
```json
{
  ...
  "service": "Insurance",
  "message": "Looking for a health insurance plan for my family."
}
```
...
## How Forms Are Currently Wired (Status: NOT connected)
...
<form
  ...
  onSubmit={(e) => {
    e.preventDefault();      // <-- currently just prevents refresh
    setSent(true);           // <-- shows "Enquiry Submitted" text
    // TODO: POST /api/enquiries with the field values
  }}
>
```

The inputs currently have **no `name` attributes**, so `FormData` would not include meaningful keys. When wiring the backend, add the `name` attributes listed in the tables above (or use a controlled component + state object) before calling `fetch`/`axios`.
